...
class Predictor:
    ...
    def html_content_parse(self, html_like_text_str):
    ...
    def _preprocess_text(self, text: str) -> str:
        ...
        # 2.去除所有空格(包括半角空格)
        text = text.replace(' ', '')
        text = self.html_content_parse(html_like_text_str=text)
        return text
    ...
    def predict(self, inputs: str | list):
        ...
        for i in range(0, len(processed_inputs), batch_size):
            ...
            batch_predictions = []
            for j, (tokens, prediction) in enumerate(zip(tokens_list, predictions)):
            ...
            processed_batches.extend(batch_inputs)

        if is_str:
            return processed_batches[0], final_predictions[0]
        return processed_batches, final_predictions
