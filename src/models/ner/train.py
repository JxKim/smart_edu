...
# 模型
model = AutoModelForTokenClassification.from_pretrained(config.MODEL_NAME,num_labels=len(config.LABELS),
...
args = TrainingArguments(
    ...
    logging_dir =str(config.LOGS_DIR/'ner'/time.strftime("%Y-%m-%d-%H-%M-%S")),
    num_train_epochs = 20,
...
# 训练
trainer = Trainer(model=model,args=args,train_dataset=train_dataset,callbacks = [early_stopping_callback],
        eval_dataset=valid_dataset,data_collator=data_collator,compute_metrics=compute_metrics)
trainer.train()
