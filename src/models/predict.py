...
class Predictor:
    ...
    def predict(self, inputs: str | list):
        ...
        for tokens, prediction in zip(tokens_list, predictions):
            prediction = prediction[:1 + len(tokens)]
...
if __name__ == '__main__':
    model = AutoModelForTokenClassification.from_pretrained(str(config.CHECKPOINT_PATH / 'best_model'))
    tokenizer = AutoTokenizer.from_pretrained(str(config.CHECKPOINT_PATH /'best_model'))
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    predictor = Predictor(model, tokenizer, device)
    text = "17_ClickHouse入门_SQL操作_Update和Delete.avi"
