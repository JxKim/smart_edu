...
from models.uie_pytorch.utils import logger, get_bool_ids_greater_than, get_span, get_id_and_prob, cut_chinese_sent, dbc2sbc
...
class PyTorchInferBackend:
    def __init__(self,
                 ...
                 device='cpu',
                 use_fp16=False):
        from models.uie_pytorch.model import UIE, UIEM
...
class UIEPredictor(object):
    ...
    def __init__(self, model, schema=['知识点'], task_path=None, schema_lang="zh", engine='pytorch', device='gpu', position_prob=0.5, max_seq_len=512, batch_size=16, split_sentence=False, use_fp16=False):
    ...
    def _single_stage_predict(self, inputs):
        ...
        sentence_ids = []
        probs = []
        for start_ids, end_ids, ids, offset_map in zip(start_ids_list, end_ids_list, input_ids.tolist(),
                                                       offset_maps.tolist()):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--task_path",
        type=str,
        default=str(config.ROOT_DIR/'src'/'models'/'uie_pytorch'/'checkpoint'/'model_best'),
...
if __name__ == '__main__':
    ...
    print(uie(
        [
            ...
            '讲解Maven项目构建工具的核心使用方法，包括依赖管理、项目结构、插件配置、多模块项目构建等内容，提升Java项目的开发效率与规范性。',
            '深入讲解MyBatis持久层框架的使用，包括XML配置、注解方式、动态SQL、缓存机制等，实现数据库操作的灵活控制。',
            '从零开始系统讲解Redis数据类型、持久化机制、事务、发布订阅、集群部署等核心知识点，并结合电商秒杀场景进行实战演练。',
            '全面讲解Spring框架核心功能，包括IoC容器、AOP编程、事务管理、整合其他组件等，奠定Java企业级开发基础。']
    ))