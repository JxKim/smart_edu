...
if __name__ == '__main__':
    # 模型
    model = AutoModelForTokenClassification.from_pretrained(config.CHECKPOINTS_DIR / 'chapter' / 'best_model')
    # 分词器
    tokenizer = AutoTokenizer.from_pretrained(config.CHECKPOINTS_DIR / 'chapter' / 'best_model')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    predictor = Predictor(model, tokenizer, device)
    text = "day01_04Java语言特性详解"
