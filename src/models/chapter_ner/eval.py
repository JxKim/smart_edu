...
# 模型
model = AutoModelForTokenClassification.from_pretrained(config.CHECKPOINTS_DIR / 'chapter' / 'checkpoint-500')

# 数据
test_dataset = load_from_disk(config.DATA_DIR_PROCESSED / 'chapter' / 'test')

# 分词器
tokenizer = AutoTokenizer.from_pretrained(config.CHECKPOINTS_DIR / 'chapter' / 'checkpoint-500')
# 数据整理器
