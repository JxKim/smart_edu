...
class ChapterInfoSync:
    def __init__(self):
        ...
        self.uie = UIEPredictor(model='uie-base',
                                task_path= str(config.ROOT_DIR /'src'/'uie_pytorch'/'checkpoint'/'course'/'model_best'),
                                schema=['KN'],
                                device='gpu')
    ...
    def sync_KN(self):
        ...
        while True:
            sql = """
                ...
                # limit 5
            """
            data = self.mysql_reader.read(sql,(32, offset))
            ...
            chapter = [item['course_introduce'] for item in data]
            uie_chapter_list = self.uie(chapter)

            tag_properties =  []
            relations =  []
            for id, course_introduce in zip(ids, uie_chapter_list):
                if not course_introduce:
                    continue
                chapter_list = self._tiqu_text(course_introduce)
