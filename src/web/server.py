    ...
    def __init__(self):
        ...
        self.neo4j_vectors = {
            'base_category_info': Neo4jVector.from_existing_index(
                ...
                keyword_index_name='category_fti',
            ...
            'base_subject_info': Neo4jVector.from_existing_index(
                ...
                keyword_index_name='subject_fti',
                search_type=SearchType.HYBRID,
            ),
            'course_info': Neo4jVector.from_existing_index(
                ...
                keyword_index_name='course_fti',
                search_type=SearchType.HYBRID,
            ),
            'chapter__info': Neo4jVector.from_existing_index(
                self.embedding_model,
                url=config.NEO4J_CONFIG["uri"],
                username=config.NEO4J_CONFIG['auth'][0],
                password=config.NEO4J_CONFIG['auth'][1],
                index_name='chapter_vector_index',
                keyword_index_name='chapter_fti',
                search_type=SearchType.HYBRID,
            ),
            ...
            'test_paper': Neo4jVector.from_existing_index(
                ...
                keyword_index_name='test_paper_fti',
                search_type=SearchType.HYBRID,
            ),
            'test_paper_question': Neo4jVector.from_existing_index(
                self.embedding_model,
                url=config.NEO4J_CONFIG["uri"],
                username=config.NEO4J_CONFIG['auth'][0],
                password=config.NEO4J_CONFIG['auth'][1],
                index_name='test_paper_question_vector_index',
                keyword_index_name='test_paper_question_fti',
                search_type=SearchType.HYBRID,
            )
...
if __name__ == '__main__':
    ...
    chat_service.chat("有哪些课程对应的teacher是谁？")