...
class ChatService:
    def __init__(self):
        self.llm = ChatDeepSeek(model='deepseek-chat', api_key=os.getenv('API_KEY'))
        self.embedding_model = HuggingFaceEmbeddings(model_name='BAAI/bge-base-zh-v1.5',
                                                     encode_kwargs={'normalize_embeddings': True})
        self.neo4j_vectors = {
            'Tag': Neo4jVector.from_existing_index(
                self.embedding_model,
                url=os.getenv('NEO4J_URI'),
                username=os.getenv('NEO4J_AUTH_USER'),
                password=os.getenv('NEO4J_AUTH_PASSWORD'),
                index_name='tag_vector_index',
                keyword_index_name='tag_full_text_index',
        ...
        self.graph = Neo4jGraph(url=os.getenv('NEO4J_URI'),
                                username=os.getenv('NEO4J_AUTH_USER'),
                                password=os.getenv('NEO4J_AUTH_PASSWORD'))
    ...
    def _generate_answer(self, question, query_result):
        prompt = """
        你是一个计算机课程介绍者，根据用户问题，以及数据库查询结果生成一段简洁、准确的自然语言回答。
                用户问题: {question}
...
if __name__ == '__main__':
    ...
    answer = chat_service.chat("人工智能都要学哪些课程")
    print(answer)
